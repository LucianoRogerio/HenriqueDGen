...
## Mixed Models Agronomic Traits

```{r}
suppressMessages(library(lme4)); suppressMessages(library(tidyverse))
library(reactable); library(here)

AgroData <- readRDS(file = here::here("data", "DadosFen2020.rds"))

```

#### Table 4. Data entry to perform the mxed model analysis for Agronomic Traits
```{r, echo = FALSE}
AgroData %>% reactable(defaultPageSize = 25, columns = list(
  FRY = colDef(format = colFormat(digits = 2, locales = "en-US")),
  AGY = colDef(format = colFormat(digits = 2, locales = "en-US")),
  HI = colDef(format = colFormat(digits = 2, locales = "en-US")),
