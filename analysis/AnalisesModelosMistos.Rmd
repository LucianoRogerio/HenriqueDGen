...
MediasFix$MeanEffect <- rownames(MediasFix)

MediasFix %<>% filter(MeanEffect %like% "control") %>%
  as.data.frame() %>% dplyr::select(MeanEffect, everything())
rownames(MediasFix) <- NULL
MediasFix$MeanEffect <- gsub(pattern = "control", replacement = "", x = MediasFix$MeanEffect)
